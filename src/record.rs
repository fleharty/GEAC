...
/// Reference-site record produced by `geac collect --emit-ref-sites`.
///
/// One row per target position per sample where the sample had **no alt reads**
/// at that position.  Carries the same depth / strand / overlap / read-context
/// metrics as an AltBase record so that carrier and non-carrier samples can be
/// compared at the same loci.  Positions are 0-based.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RefBase {
    pub sample_id: String,
    pub chrom: String,
    ...
    pub str_period: Option<i32>,
    pub str_len: Option<i32>,
    pub gnomad_af: Option<f32>,
    pub input_checksum_sha256: Option<String>,
}

/// Per-read record at a reference site, produced alongside RefBase when
/// `geac collect --emit-ref-sites` is run.
///
/// One row per read (fragment) covering the target position, regardless of
/// whether that read supports the reference allele or a rare error allele.
/// Positions are 0-based.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RefRead {
    pub sample_id: String,
    ...
    /// Bases before the queried position in read sequence order
    pub n_before_alt: i32,
    /// Bases after the queried position in read sequence order
    pub n_after_alt: i32,
