    ...
    fn parse(content: &str) -> Result<Self> {
        ...
        for line in content.lines() {
            ...
            let (start, end, name) = if is_interval_list {
                ...
                let s: u32 = fields[1]
                    .parse()
                    .with_context(|| format!("invalid start coordinate in BED: '{}'", fields[1]))?;
                let e: u32 = fields[2]
                    .parse()
                    .with_context(|| format!("invalid end coordinate in BED: '{}'", fields[2]))?;
                let name = fields
                    .get(3)
                    .map(|n| n.trim().to_string())
                    .filter(|n| !n.is_empty());
        ...
        named_raw.sort_unstable_by(|a, b| a.chrom.cmp(&b.chrom).then(a.start.cmp(&b.start)));
