...
fn records_to_batch(records: &[IntervalRecord], schema: Arc<Schema>) -> Result<RecordBatch> {
    ...
    let start: ArrayRef = Arc::new(Int64Array::from_iter_values(
        records.iter().map(|r| r.start),
    ));
    ...
    let interval_name: ArrayRef = Arc::new(StringArray::from(
        records
            .iter()
            .map(|r| r.interval_name.as_deref())
            .collect::<Vec<_>>(),
    ));
    ...
    let n_bases: ArrayRef = Arc::new(Int32Array::from_iter_values(
        records.iter().map(|r| r.n_bases),
    ));
    let mean_depth: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.mean_depth),
    ));
    let median_depth: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.median_depth),
    ));
    ...
    let frac_at_1x: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.frac_at_1x),
    ));
    let frac_at_10x: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.frac_at_10x),
    ));
    let frac_at_20x: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.frac_at_20x),
    ));
    let frac_at_30x: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.frac_at_30x),
    ));
    let frac_at_50x: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.frac_at_50x),
    ));
    let frac_at_100x: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.frac_at_100x),
    ));

    let mean_gc_content: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.mean_gc_content),
    ));
    let mean_mapq: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.mean_mapq),
    ));
    let mean_frac_mapq0: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.mean_frac_mapq0),
    ));
    let mean_frac_dup: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.mean_frac_dup),
    ));
    let mean_frac_overlap: ArrayRef = Arc::new(Float32Array::from_iter_values(
        records.iter().map(|r| r.mean_frac_overlap),
    ));
