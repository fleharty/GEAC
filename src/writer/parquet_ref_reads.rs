...
use crate::record::RefRead;

pub fn write_parquet(records: &[RefRead], output: &Path) -> Result<()> {
    let schema = ref_read_schema();
...
fn records_to_batch(records: &[RefRead], schema: Arc<Schema>) -> Result<RecordBatch> {
