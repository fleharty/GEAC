...
fn records_to_batch(records: &[RefBase], schema: Arc<Schema>) -> Result<RecordBatch> {
    ...
    let ref_count: ArrayRef =
        Arc::new(Int32Array::from_iter_values(records.iter().map(|r| r.ref_count)));
    let fwd_ref_count: ArrayRef =
        Arc::new(Int32Array::from_iter_values(records.iter().map(|r| r.fwd_ref_count)));
    let rev_ref_count: ArrayRef =
        Arc::new(Int32Array::from_iter_values(records.iter().map(|r| r.rev_ref_count)));
    let overlap_depth: ArrayRef =
        Arc::new(Int32Array::from_iter_values(records.iter().map(|r| r.overlap_depth)));
    let overlap_ref_agree: ArrayRef =
        Arc::new(Int32Array::from_iter_values(records.iter().map(|r| r.overlap_ref_agree)));
    ...
    let homopolymer_len: ArrayRef = Arc::new(Int32Array::from(
        records.iter().map(|r| r.homopolymer_len).collect::<Vec<_>>(),
    ));
    ...
    RecordBatch::try_new(
        schema,
        vec![
            sample_id, chrom, pos, ref_allele,
            total_depth, fwd_depth, rev_depth,
            ref_count, fwd_ref_count, rev_ref_count,
            overlap_depth, overlap_ref_agree,
            read_type, pipeline,
            batch, label1, label2, label3,
            on_target, gene,
            homopolymer_len, str_period, str_len,
            gnomad_af, input_checksum_sha256,
        ],
    )
