...
| `--gene-padding <BP>` | `0` | Symmetrically extend each gene body by `BP` bp on both sides before k-mer extraction (lower bound clamped to 0, upper bound to the contig length). Captures fusion breakpoints in flanking introns/UTRs just outside the annotated transcript. The `genes` table keeps the **true** transcript bounds; only the extraction window widens, and the value is recorded in `meta` (`gene_padding`). Larger padding raises cross-gene/repeat-k-mer collisions, so it interacts with the cross-gene dedup and the `--edit-distance-filter`/`--max-genome-copies` uniqueness passes (which still drop the newly shared/common k-mers). |
...
| `--min-unique-anchor-reads <N>` | `0` (off) | Copy-aware specificity filter. Keeps a call `filter=PASS` only if ≥ `N` supporting fragments are anchored by a **genome-unique** (`genome_copies==1`) k-mer on the **higher-uniqueness partner** of the pair; otherwise tags it `filter=no_unique_anchor` (rows kept, not dropped; only calls still PASS are touched, so prior `pon`/`chimera`/`samelocus` tags win). Targets repeat/segdup/pseudogene partners (e.g. a gene resident in a macrosatellite): their multi-copy k-mers match reads genome-wide and fabricate a high-support call in nearly every sample, but those fabricated calls have no unique-anchored junction. The check is **asymmetric** — only the partner with more genome-unique k-mers (chosen per call from the index's per-gene unique-k-mer counts) must be unique-anchored, since a repeat partner cannot be. Provides specificity; pair with a repeat-tolerant / soft-clip anchor for sensitivity. Requires an index built with `--check-genome-uniqueness` (a `genome_copies` column); errors clearly otherwise. |
| `--emit-unique-anchor` | `false` | Emit the per-call `n_unique_anchored` count **without** applying the unique-anchor filter. Turns on the same per-k-mer uniqueness tracking as `--min-unique-anchor-reads` (so it also needs a `--check-genome-uniqueness` index) and populates the `n_unique_anchored` output column, but tags nothing. Lets a **single run record the raw evidence** so any threshold can be applied offline as `n_unique_anchored / supporting_reads`, instead of re-running the caller once per candidate threshold. Implied when `--min-unique-anchor-reads > 0`. |
| `--min-breakpoint-distance <BP>` | — | Same-locus / adjacency filter. Tags a fusion `filter=samelocus` (rows kept) when **both** breakpoints fall on the same chromosome within `BP` bp. Both partners localizing to one spot is the signature of single-locus paralog leakage: reads from an *unindexed* paralog (e.g. GNA12, chr7) carry k-mers shared with two indexed cousins (GNA13 on chr17, GNA11 on chr19) and split between them, fabricating a fusion whose breakpoints both sit at the GNA12 locus (Δ ≈ a few bp). That fake breakpoint is tight and reproducible, so `--max-breakpoint-std` passes it — this catches it on geometry instead. A genuine fusion places partners on different chromosomes, or far apart on one. Recommended: `10000` (≫ a fragment, ≪ inter-gene distance). **Exemption:** a call with strong *independent* (concordant, non-spanning) support on both partners is kept — single-locus leakage is spanning-read dominated (one molecule carries both partners' k-mers, so subtracting spanning reads collapses it), whereas a real translocation whose breakpoint is buried in a repeat is seen only through concordant pairs and co-localizes legitimately. Triggers the same second BAM pass as `--breakpoints-output`. Default: disabled. |
...
pon_total_samples, max_pon_supporting_reads, filter, partner_order,
n_unique_anchored`. The three `*pon*` columns are `0`/`0`/`NA` unless `--fusion-pon`
is given. `n_unique_anchored` is the count of supporting fragments anchored by a
genome-unique k-mer on the higher-uniqueness partner (denominator =
`supporting_reads`); it is `NA` unless uniqueness tracking ran
(`--min-unique-anchor-reads > 0` or `--emit-unique-anchor` with a
`--check-genome-uniqueness` index). `filter` is `PASS` by default, `pon`
when `--max-pon-samples` flags a PoN-recurrent call, `chimera` when
`--max-breakpoint-std` rejects a call for lacking breakpoint consensus,
`samelocus` when `--min-breakpoint-distance` rejects a call whose breakpoints
co-localize to one locus (single-locus paralog leakage), or `no_unique_anchor`
when `--min-unique-anchor-reads` flags a call lacking a genome-unique anchor.
